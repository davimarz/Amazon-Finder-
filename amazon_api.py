...
RE_ASIN_URL = re.compile(r"(?:/dp/|/gp/product/|/d/)([A-Z0-9]{10})(?:[/?&#]|$)", re.IGNORECASE)
RE_ASIN_EXACT = re.compile(r"^[B0-9][A-Z0-9]{9}$", re.IGNORECASE)
...
def ottieni_offerte_avanzate(
    ...
    # Riconoscimento ASIN sicuro: solo se è un URL esplicito o una stringa ASIN esatta (senza spazi)
    is_url = "amazon." in clean_keyword.lower() and "/dp/" in clean_keyword.lower()
    url_match = RE_ASIN_URL.search(clean_keyword)
    exact_asin = clean_keyword.upper() if RE_ASIN_EXACT.match(clean_keyword) else None

    target_asin = url_match.group(1).upper() if (is_url and url_match) else exact_asin
    ...
    # Ricerca catalogo multi-prodotto (minimo 10 schede richieste)
    ...
    # Se l'API restituisce i prodotti richiesti per la pagina, li ritorna direttamente
    if api_products and len(api_products) >= item_count:
        return api_products[:item_count]

    # Fallback o completamento dei risultati fino a raggiungere item_count
    fallback_items = _search_html_fallback(
        query,
        sort_type,
        partner_tag,
        solo_spedizione_gratuita,
    ...
    if not api_products:
        return fallback_items[:item_count]

    # Unisce i prodotti dell'API con il fallback per garantire sempre 10 prodotti a pagina
    seen = {p["asin"] for p in api_products}
    seen_titles = {re.sub(r'[^a-zA-Z0-9]', '', str(p.get("titolo", "")).lower())[:50] for p in api_products}
    _append_unique(api_products, fallback_items, seen, seen_titles)
